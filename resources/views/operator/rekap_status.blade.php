...
<div class="p-4 sm:ml-64">
    <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 mt-14">
        ...
        <form class="flex items-center" action="{{ route('operator.viewRekapStatus') }}" method="GET">            
            ...
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tr class="text-center bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover-bg-gray-600">
                        <td class="px-6 py-4">
                            <a href="{{ route('operator.viewDaftarAktif', $mhsData[0]->angkatan) }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">{{ $aktif }}</a>
                        </td>
                        <td class="px-6 py-4">
                            <a href="{{ route('operator.viewDaftarCuti', $mhsData[0]->angkatan) }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">{{ $cuti }}</a>
                        </td>
                        <td class="px-6 py-4">
                            <a href="{{ route('operator.viewDaftarMangkir', $mhsData[0]->angkatan) }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">{{ $mangkir }}</a>
                        </td>
                        <td class="px-6 py-4">
                            <a href="{{ route('operator.viewDaftarDO', $mhsData[0]->angkatan) }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">{{ $do }}</a>
                        </td>
                        <td class="px-6 py-4">
                            <a href="{{ route('operator.viewDaftarUndurDiri', $mhsData[0]->angkatan) }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">{{ $undurDiri }}</a>
                        </td>
                        <td class="px-6 py-4">
                            <a href="{{ route('operator.viewDaftarLulus', $mhsData[0]->angkatan) }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">{{ $lulus }}</a>
                        </td>
                        <td class="px-6 py-4">
                            <a href="{{ route('operator.viewDaftarMeninggal', $mhsData[0]->angkatan) }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">{{ $md }}</a>
                        </td>
                    </tr>
                    </tbody>
                </table>
                <div class="p-3">        
                    <a href="{{ route('operator.cetakRekapStatus', [$mhsData[0]->angkatan]) }}" type="button" class="p-2.5 ml-2 text-sm font-medium text-white bg-blue-700 rounded-lg border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Cetak PDF</a>
                </div>
            </div>
        </div>
    </div>
